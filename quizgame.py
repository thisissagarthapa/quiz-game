...
quizList=[Quize(questions[0],'a'),
          Quize(questions[1],'b'),
          Quize(questions[2],'c'),]

def show(quizList):
    for i in quizList:
        ask=input(f"{i.questions}\n enter your answer :")
        if ask==i.answer:
            marks+=1
        else:
            print(f"you choose wrong answer,your correct answer is {i.answer} ")
    print(f"your marks is{marks/{len(questions)}}")
show(quizList)